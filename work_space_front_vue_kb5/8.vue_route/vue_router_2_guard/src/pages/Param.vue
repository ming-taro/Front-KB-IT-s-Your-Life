...
// useRoute(route) : 해당 페이지의 url 정보를 담고 있는 객체(로컬정보!)
//                   -> 현재 페이지의 path, query, param 정보를 조회 할 때 사용!
// useRouter(router) : router/index에서 생성한 router 정보로 모든 라우팅 정보를 담은 객체
//                     -> 라우터 전용 메소드 활용 ex) push, go, back, replace

const route = useRoute();
// route 활용 예제
// console.log(this.$route); // <- 에러 발생
...
// back() : go(-1)
// forward() : go(1)
// push(to) : 지정된 경로로 이동, history가 유지된다! ★★★★★
...
const goPush = () => {
    ...
    // router.push({path: '/about'}); // 객체로 요청하는 방법
}
