...
const router = createRouter({
    ...
    routes: [
        ...
        { path: '/members/:id', name:'members/id', component: MemberInfo,
            // routes 셋팅중 컴포넌트 개별 가드를 세팅할 수 있다!
...
// router.addRoute([]); <- router를 따로 추가할 수도 있음
...
router.beforeEach((to, from) => {
    ...
    if (to.path == '/param') {
        ...
        return { path: to.path, query: {}, params: to.params }; // 쿼리를 제거하고 다시 라우팅
    }

    // return false; // 라우팅이 되지 않는 코드! -> 에러처리가 되지 않으므로 없어야한다!
});
